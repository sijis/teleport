...
func (u *HostUserManagement) DeleteAllUsers() error {
	...
	sessions, err := u.client.GetActiveSessionTrackers(u.ctx)
	if err != nil {
		return trace.Wrap(err)
	}
...
outer:
	for _, name := range users {
		// delete all users is only used for periodic deletes and
		// as a result shouldnt try to delete currently active
		// sessions as it could cause a race between logging in
		// and user deletion.
		for _, sess := range sessions {
			state := sess.GetState()
			if (state == types.SessionState_SessionStatePending ||
				state == types.SessionState_SessionStateRunning) &&
				sess.GetAddress() == u.uuid &&
				sess.GetLogin() == name {
				continue outer
			}
		}
